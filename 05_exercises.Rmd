...
## Garden data
  ...
  * From the `garden_harvest` data, filter the data to the tomatoes and find the *daily* harvest in pounds for each variety.  **YES**
  * Then, for each variety, find the cumulative harvest in pounds.  **YES**
  * Add animation to reveal the plot over date. **YES**
...
tomatoes_cum <- garden_harvest %>% 
  ...
  ggplot(aes(x = date, y = cum_wt_lbs, color = fct_reorder(variety, cum_wt_lbs))) + #wdym most on the bottom?
  geom_line() +
...
## Maps, animation, and movement!
  ...
  Requirements:
  * Plot on a map using `ggmap`.  **YES**
  * Show path up until the current point.   **YES**
  * Color the path according to elevation.  **YES**
  * Show the time in the subtitle.  *maybe*
...
bikeride <- ggmap(mallorca_map) +
  ...
  geom_image(aes(image = picture), #why isnt it working?? looked at bat example too and just copied and modified
             size = 0.3) +
